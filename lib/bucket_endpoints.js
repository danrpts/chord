...
function partition (call, cb) {
  ...
  var newPredcessorId = call.request.id;

  // invalid request
  if (!Buffer.isBuffer(newPredcessorId)) {
    return cb(new Error());
  }
  ...
  for (let key in this.hashtable) {
    ...
    let id = Buffer.from(key, 'hex');

    if (id.compare(newPredcessorId) === 0
      || isBetween(id, newPredcessorId, this.id)) {
...
module.exports = { get, set, del, all, partition }
