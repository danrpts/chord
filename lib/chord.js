...
const FINGER_BASE = Array(FINGER_LENGTH).fill(0)
...
const fixFinger = (() => {
  ...
  return async function fixFingerClosure() {
    ...
    try {
      // generate finger id
      const id = bignum.fromBuffer(this.id)
                       .add(FINGER_BASE[i])
                       .mod(MAX_ID)
                       .toBuffer();
...
const fixSuccessor = async function fixSuccessor() {
  ...
  do {
    try {
      ...
      const down = _.chain(this.successor).difference(newSuccessor).without(this.address).value();

      // record successors being added
      const up = _.chain(newSuccessor).difference(this.successor).without(this.address).value();

      // fully adopt the sucessor list
      this.successor = newSuccessor;

      // successor list has been refreshed and immediate is alive
      done = true;
...
class Peer extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.server.addService(CHORD_PROTO.service,
  ...
  async join(host) {
    ...
    try {
      ...
      // find successor
      const lookupResponse = await doRPC(host, 'lookup', { id: toSHA1(this.address) });
  ...
  /**
   * Generator that yields the next live
   * @return {string}
   */
  * getSuccessor() {
