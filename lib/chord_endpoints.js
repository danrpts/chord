...
async function lookup(call, cb) {
  ...
  if (isBetween(this.id, keyId, successorId) || keyId.compare(successorId) === 0) {
    try {
  ...
  // II. Recursive Case
    ...
    try {
      const response = await doRPC(successor, 'lookup', { id: keyId, counter });
  ...
  return cb(new Error(`invalid successor ${successor} on ${this.address} hop ${counter}`));
}
